...
@Component
public class NotifyDateCommandHandler implements CommandHandler {
    ...
    @Autowired
    public NotifyDateCommandHandler(WebexFacade webexFacade,
        ...
        if (text.length >= 2) {
            ...
            if (persons.size() > 0) {
                for (var person : persons) {
                    person.setIgnoring(false);
                    personService.save(person);
                }

                webexFacade.sendDirectMessage(message.getPersonEmail(), messageUtil.getText(ReplyLocaleText.REPLY_COMMAND_NOTIFY_DATE, groupName));
            } else {
            ...
            webexFacade.sendDirectMessage(message.getPersonEmail(), messageUtil.getText(ReplyLocaleText.REPLY_COMMAND_NOTIFY_DATE_BAD_FORMAT,
                    messageUtil.getText(CommandLocaleText.COMMAND_NOTIFY_DATE), messageUtil.getText(CommandLocaleText.COMMAND_ARGUMENT_ALL)));
        }
    }

    @Override
    public CommandType getCommandType() {
        return CommandType.NOTIFY_DATE;
    }
}
